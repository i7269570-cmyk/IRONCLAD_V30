...
def run_pipeline(paths: dict, strategy_path: str, state_path: str, evidence_path: str, state: Dict[str, Any], system_config: Dict[str, Any]):
    try:
        ...
        # ------------------------------------------------------------
        if mode == "TRADE":
            # [Contract Fix] regime_filter용 스냅샷 리스트 생성 (List[Dict])
            market_data = []
            ...
            if evaluate_market_regime(market_data, strategy_path):
                ...
                for strat in active_strategies:
                    signals = generate_signals(data_bundle, os.path.join(BASE_DIR, strat["path"]), strat["id"], state, system_config)
        ...
        if mode in ["TRADE", "NO_ENTRY", "FORCE_EXIT"]:
            ...
            for item in exit_list: exit_results[item["symbol"]] = item
...
if __name__ == "__main__":
    paths = {
        ...
        "LOCKED": os.path.join(BASE_DIR, "LOCKED") # Gate 및 Handler 필수 경로
    }
    with open(paths["STATE"], "r", encoding="utf-8") as f: current_state = json.load(f)
    with open(os.path.join(BASE_DIR, "LOCKED", "system_config.yaml"), "r", encoding="utf-8") as f: sys_config = yaml.safe_load(f)
